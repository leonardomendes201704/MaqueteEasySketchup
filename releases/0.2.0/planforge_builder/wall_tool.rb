module LeonardoLabs
  module PlanForgeBuilder
    class WallTool
      ...
      PROMPT_DRAWING = 'Clique para concluir a parede. Enter fecha o comodo e cria o piso. Esc cancela o tracado.'.freeze
      ...
      def onMouseMove(_flags, x, y, view)
        if drawing?
          ...
          @preview_data = @preview_end ? GeometryBuilder.preview_data(@last_point, @preview_end, Settings.to_h) : nil
      ...
      def finalize_segment(endpoint)
        ...
        GeometryBuilder.build_wall(model, @last_point, endpoint, settings)
        if closed_room && should_create_floor?(contour, settings)
          GeometryBuilder.build_floor(model, contour, settings)
        end
