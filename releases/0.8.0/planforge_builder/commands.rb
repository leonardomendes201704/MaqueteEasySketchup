module LeonardoLabs
  module PlanForgeBuilder
    module CommandRegistry
      ...
      def room_command
        @room_command ||= begin
          command = UI::Command.new('Gerar comodo') { PlanForgeBuilder.prompt_room_tool }
          assign_icons(command, 'room')
          command.menu_text = 'Gerar comodo'
          command.tooltip = 'Gerar comodo retangular'
          command.status_bar_text = 'Abre o modal de medidas e ativa o posicionamento do comodo retangular.'
          command
        end
      end
      ...
      def baseboard_command
        @baseboard_command ||= begin
          command = UI::Command.new('Gerar rodape') { PlanForgeBuilder.generate_baseboards_for_selection }
          assign_icons(command, 'baseboard')
