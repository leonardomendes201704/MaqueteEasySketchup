  ...
  module PlanForgeBuilder
    module BaseboardBuilder
      ...
      def build_for_room(model, contour_points, wall_groups, settings = Settings.to_h, room_token = nil, replace_existing = true)
        ...
        return group if baseboard_height <= TOLERANCE || baseboard_depth <= TOLERANCE
      ...
      def build_wall_baseboards(entities, wall_group, wall, inner_start, inner_end, interior_offset, height, depth)
        ...
        free_intervals(wall_group, wall, range_start, range_end, height).each do |interval_start, interval_end|
      ...
      def free_intervals(wall_group, wall, range_start, range_end, baseboard_height)
        blockers = GeometryBuilder.opening_records(wall_group).each_with_object([]) do |opening, intervals|
          next unless opening[:bottom_z] <= (wall[:base_z] + baseboard_height + TOLERANCE)
