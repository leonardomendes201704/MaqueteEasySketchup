    ...
    module BaseboardBuilder
      ...
      def build_for_room(model, contour_points, wall_groups, settings = Settings.to_h, room_token = nil, replace_existing = true)
        ...
        if baseboard_height <= TOLERANCE || baseboard_depth <= TOLERANCE
          LayerManager.apply_to_entity(group, :baseboard)
      ...
      def free_intervals(wall_group, wall, range_start, range_end, base_z, baseboard_height)
        blockers = openings_for_baseboard(wall_group).each_with_object([]) do |opening, intervals|
      ...
      def openings_for_baseboard(wall_group)
        return GeometryBuilder.opening_records(wall_group) unless GeometryBuilder.proxy_wall?(wall_group)

        shared_id = GeometryBuilder.shared_wall_id(wall_group)
        return [] unless shared_id

        host_group = wall_group.model.active_entities.grep(Sketchup::Group).find { |group| group.persistent_id == shared_id }
        host_group ? GeometryBuilder.opening_records(host_group) : []
      end
    end
  end
end
