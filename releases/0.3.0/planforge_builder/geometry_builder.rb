  ...
  module PlanForgeBuilder
    module GeometryBuilder
      ...
      def door_preview_data(group, world_point, settings = Settings.to_h)
        ...
        sanitized = Settings.sanitize(settings)
        door_width = sanitized[:door_width_cm].to_f.cm
        door_height = [sanitized[:door_height_cm].to_f.cm, wall[:height]].min
        return nil if door_width <= TOLERANCE || door_height <= TOLERANCE

        clearance = [1.cm.to_f, wall[:thickness] / 2.0].max
        min_distance = clearance + (door_width / 2.0)
        max_distance = wall[:length] - clearance - (door_width / 2.0)
        ...
        local_face = build_opening_face(center_point, wall[:axis], wall[:left_axis], surface_offset, door_width, wall[:base_z], door_height)
      ...
      def cut_door_opening(group, preview_or_world_point, settings = Settings.to_h)
        ...
        raise ArgumentError, 'Nao foi possivel posicionar a porta nesta parede.' unless preview

        face = group.entities.add_face(preview[:local_face])
        raise ArgumentError, 'Nao foi possivel criar o recorte da porta nesta parede.' unless face
      ...
      def wall_data(group)
        ...
        thickness = group.get_attribute(PLUGIN_ID, 'wall_thickness_cm', 0.0).to_f.cm
        height = group.get_attribute(PLUGIN_ID, 'wall_height_cm', 0.0).to_f.cm
        alignment = group.get_attribute(PLUGIN_ID, 'alignment', 'center')
      ...
      def build_opening_face(center_point, axis, left_axis, surface_offset, width, base_z, height)
        start_point = offset_point(center_point, axis, -(width / 2.0))
        lower_start = offset_point(start_point, left_axis, surface_offset)
        lower_start.z = base_z
        lower_end = offset_point(lower_start, axis, width)
        upper_end = lower_end.offset(Z_AXIS, height)
        upper_start = lower_start.offset(Z_AXIS, height)
