...
module Prawn
  ...
  class Font
    ...
    def self.load(document, src, options = {})
      ...
      when 'ttf'
        load_ttf_support!
        Fonts::TTF.new(document, src, options)
      when 'otf'
        load_otf_support!
        Fonts::OTF.new(document, src, options)
      when 'dfont'
        load_dfont_support!
        Fonts::DFont.new(document, src, options)
      when 'ttc'
        load_ttc_support!
        Fonts::TTC.new(document, src, options)
    ...
    def self.load_ttf_support!
      return if defined?(Fonts::TTF)

      require_relative 'fonts/ttf'
    end

    def self.load_otf_support!
      return if defined?(Fonts::OTF)

      require_relative 'fonts/otf'
    end

    def self.load_dfont_support!
      return if defined?(Fonts::DFont)

      require_relative 'fonts/dfont'
    end

    def self.load_ttc_support!
      return if defined?(Fonts::TTC)
