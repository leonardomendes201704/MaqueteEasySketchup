...
module LeonardoLabs
  module PlanForgeBuilder
    ...
    EXTENSION_VERSION = '0.9.0'.freeze unless const_defined?(:EXTENSION_VERSION)
...
LeonardoLabs::PlanForgeBuilder.safe_require('metric_units')
...
module LeonardoLabs
  module PlanForgeBuilder
    ...
    def prompt_room_tool
      values = UI.inputbox(
        ['Largura interna do comodo (m)', 'Profundidade interna do comodo (m)'],
        [
          MetricUnits.meter_field_value(Settings.current[:room_width_cm].to_f.cm),
          MetricUnits.meter_field_value(Settings.current[:room_depth_cm].to_f.cm)
        ],
        'Gerar comodo (metros)'
      )
      return false unless values

      width = coerce_metric_length(values[0])
      depth = coerce_metric_length(values[1])
