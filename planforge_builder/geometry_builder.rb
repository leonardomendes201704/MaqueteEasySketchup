  ...
  module PlanForgeBuilder
    module GeometryBuilder
      ...
      def rebuild_wall(group, start_point, end_point, settings = Settings.to_h, prev_point = nil, next_point = nil)
        ...
        group.set_attribute(PLUGIN_ID, 'wall_proxy', false)
        group.set_attribute(PLUGIN_ID, 'shared_wall_id', nil)
      ...
      def rebuild_proxy_wall(group, start_point, end_point, settings = Settings.to_h, shared_wall_id = nil)
        sanitized = Settings.sanitize(settings)
        entities = group.entities
        existing = entities.to_a
        entities.erase_entities(existing) unless existing.empty?

        group.name = 'PlanForge Shared Wall Proxy'
        group.set_attribute(PLUGIN_ID, 'entity_type', 'wall')
        group.set_attribute(PLUGIN_ID, 'wall_proxy', true)
        group.set_attribute(PLUGIN_ID, 'shared_wall_id', shared_wall_id)
        store_entity_settings(group, sanitized)
        store_wall_metadata(group, start_point, end_point)
      ...
      def build_floor(model, contour_points, settings = Settings.to_h)
        ...
        LayerManager.apply_to_entity(group, :floor)
      ...
      def room_interior_margin(settings = Settings.to_h)
        sanitized = Settings.sanitize(settings)
        left_extent, = extents_for_alignment(sanitized[:alignment], sanitized[:wall_thickness_cm].to_f.cm)
        left_extent
      end
      ...
      def proxy_wall?(entity)
        wall_group?(entity) && entity.get_attribute(PLUGIN_ID, 'wall_proxy', false)
      end

      def physical_wall_group?(entity)
        wall_group?(entity) && !proxy_wall?(entity)
      end

      def shared_wall_id(entity)
        return nil unless entity.respond_to?(:get_attribute)

        value = entity.get_attribute(PLUGIN_ID, 'shared_wall_id')
